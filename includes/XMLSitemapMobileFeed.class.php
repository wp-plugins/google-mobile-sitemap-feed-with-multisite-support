...
class XMLSitemapMobileFeed {
	public static function go() {		
		...
		if ( $wpdb->blogid && function_exists( 'get_site_option' ) && get_site_option( 'tags_blog_id' ) == $wpdb->blogid ) {
			...
			add_action( 'do_feed_sitemap-mobile', array( __CLASS__, 'load_template_sitemap_mobile' ), 10, 1 ); // FEED
		...
		register_deactivation_hook( XMLSMF_PLUGIN_DIR . '/xml-sitemap.php', array( __CLASS__, 'deactivate' ) ); // DE-ACTIVATION
	}

	// set up the mobile sitemap template
	public static function load_template_sitemap_mobile() {
		load_template( XMLSMF_PLUGIN_DIR . '/includes/feed-sitemap-mobile.php' );
	}
	...
	public static function EnviaPing() {
		$ping = array( 
			"http://www.google.com/webmasters/sitemaps/ping?sitemap=" . urlencode( home_url( '/' ) . "sitemap-mobile.xml" ), 
			"http://www.bing.com/webmaster/ping.aspx?siteMap=" . urlencode( home_url( '/' ) . "sitemap-mobile.xml" ) 
		);
		$options['timeout'] = 10;
		foreach( $ping as $url ) {
